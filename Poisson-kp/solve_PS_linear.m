...
solve_PS_linear(mesh,kx,ky,esheet,hsheet,unrelax,mate_sys)
%
...
[EFn,FVAL,EXITFLAG] = ...
fminbnd(@(EFn) f_charge_e(mesh,kgrid,SB1,EFn,esheet), 2.5,5,OPTIONS);
[EFh,FVAL,EXITFLAG] = ...
fminbnd(@(EFh) f_charge_h(mesh,kgrid,SB2,EFh,hsheet),-1,0.5,OPTIONS);
...
%phi=Amat\rho;
[LL,UU,PP,QQ]=lu(Amat);
phi= -QQ*(UU\(LL\(PP*rho)));                                               % interprenation -phi! not  - rho! (band-diagram use e-energy)
phi_xc=(phi(1:end-1) + phi(2:end))/2;
%-------------------------------------------------------------solve poisson
%
%--------------------------------------------------------------------update
%update = phi(2:end)'-(mesh.evb-mesh.evb0);
update = phi_xc'-(mesh.evb-mesh.evb0);
mesh.evb= mesh.evb + unrelax * update ;
mesh.ecb= mesh.evb;
%--------------------------------------------------------------------update
end